...
const FullCardItem = ({ cardItem, close }: IFullCard): JSX.Element => {
  const { data: card, isLoading } = useGetCardQuery(cardItem.id);

  const [date] = new Date(cardItem.created_at).toISOString().split('T');
        ...
        <div className={styles.card} onClick={handleClick}>
          <div className={styles.card__photo}>
            <img
              ...
              className={styles.card__photo_img}
            />
          </div>
